// Hero Section Content & Configuration

export const HERO_CONFIG = {
  headline: "Designing Excellence Since 1983",
  subheadline: "Trusted by 500+ Global Brands",
  description: "Four decades of premium apparel manufacturing excellence. From concept to export, we deliver quality that exceeds expectations.",
  cta_primary: "Explore Our Work",
  cta_secondary: "Get in Touch",
};

// Updated background carousel images (manufacturing facility showcase)
export const CAROUSEL_IMAGES = [
  "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=1600&q=80",
  "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=1600&q=80",
  "https://images.unsplash.com/photo-1487215078519-e21cc028cb29?w=1600&q=80",
  "https://images.unsplash.com/photo-1605000797499-95a51c5269ae?w=1600&q=80",
  "https://images.unsplash.com/photo-1565514020179-026b92b2d70b?w=1600&q=80",
  "https://images.unsplash.com/photo-1542744173-8e7e53415bb0?w=1600&q=80",
  "https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?w=1600&q=80",
];

export const OVERVIEW_CONFIG = {
  headline: "Who We Are",
  title: "Boutique International",
  description: "A global leader in premium apparel manufacturing, serving the world's most trusted fashion brands. Our commitment to quality, innovation, and sustainability drives everything we do.",
  points: [
    { icon: "✓", text: "ISO 9001 Certified Manufacturing" },
    { icon: "✓", text: "OEKO-TEX & GOTS Compliant" },
    { icon: "✓", text: "40+ Years of Industry Excellence" },
    { icon: "✓", text: "500+ Global Brand Partnerships" },
  ],
};

// Updated statistics based on new data (Image 2)
export const FEATURES = [
  {
    number: "40",
    label: "Years of Excellence",
    description: "Four decades of manufacturing expertise and innovation",
    suffix: "+",
  },
  {
    number: "150",
    label: "Global Brands & Clients",
    description: "Trusted by leading brands across the world",
    suffix: "+",
  },
  {
    number: "50",
    label: "Million Garments Exported",
    description: "Millions of garments shipped to 25+ countries",
    suffix: "M+",
  },
  {
    number: "98",
    label: "On-Time Delivery Rate",
    description: "Industry-leading reliability and consistency",
    suffix: "%",
  },
  {
    number: "3000",
    label: "Skilled Professionals",
    description: "Expert craftspeople dedicated to perfection",
    suffix: "+",
  },
  {
    number: "25",
    label: "Countries Served",
    description: "Global reach with local expertise",
    suffix: "+",
  },
];
